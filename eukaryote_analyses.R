...
org <- readRDS(here::here("asv_tables","euk1_dada2_otutable.rds"))
lts <- readRDS(here::here("asv_tables","euk2_dada2_otutable.rds"))
...
saveRDS(tab1, here::here("asv_tables","euk_combined_otutable.rds"))

tab1 <- readRDS(here::here("asv_tables","euk_combined_otutable.rds"))
...
uniquesToFasta(getUniques(tab1), fout=here::here("taxonomic_assignments","euk_otus.fasta"), ids=paste0("Seq", seq(length(getUniques(tab1)))))
# blastn -db ~/data/reference_databases/pr2/pr2_version_4.13.0_18S_dada2.fasta -max_target_seqs 10 -num_threads 50 -outfmt "6 std qlen qcovs sgi sseq ssciname staxid" -out euk_blasthits.txt -qcov_hsp_perc 80 -perc_identity 50 -query euk_otus.fasta
...
saveRDS(tab1, here::here("asv_tables","euk_combined_adjusted.rds"))

#taxonomy
tar_tax <- read.csv(here::here("taxonomic_assignments","euk_blasthits.txt"),sep='\t',header=F,as.is=TRUE)
tar_tax <- tar_tax[,1:3]
names(tar_tax) <- c("otu_id","SH_header","pident")

#### adjust annotation ####
#split taxon string into elements
tar_tax$kingdom <- str_split_fixed(tar_tax$SH_header, ";", 8)[,1]
tar_tax$supergroup <- str_split_fixed(tar_tax$SH_header, ";", 8)[,2]
tar_tax$division <- str_split_fixed(tar_tax$SH_header, ";", 8)[,3]
tar_tax$class <- str_split_fixed(tar_tax$SH_header, ";", 8)[,4]
tar_tax$order <- str_split_fixed(tar_tax$SH_header, ";", 8)[,5]
tar_tax$family <- str_split_fixed(tar_tax$SH_header, ";", 8)[,6]
tar_tax$genus <- str_split_fixed(tar_tax$SH_header, ";", 8)[,7]
#tar_tax$species <- str_split_fixed(tar_tax$SH_header, ";", 8)[,8]

tar_tax2 <- tar_tax %>% add_count(otu_id, name="total_n") %>% #calculate scores per taxonomic level
...
otunumber <- as.numeric(gsub("Seq","",tar_tax2$otu_id))
table_dim <- dim(tab1)[2]
dada2_format_tax <- matrix("unidenfified", nrow = table_dim, ncol = 7)
dada2_format_tax[otunumber,1] <- tar_tax2$kingdom
dada2_format_tax[otunumber,2] <- tar_tax2$supergroup
dada2_format_tax[otunumber,3] <- tar_tax2$division
dada2_format_tax[otunumber,4] <- tar_tax2$class
dada2_format_tax[otunumber,5] <- tar_tax2$order
dada2_format_tax[otunumber,6] <- tar_tax2$family
dada2_format_tax[otunumber,7] <- tar_tax2$genus
#dada2_format_tax[otunumber,8] <- tar_tax2$species

colnames(dada2_format_tax) <- c("kingdom", "supergroup","division", "class", "order", "family", "genus")

saveRDS(dada2_format_tax,here::here("taxonomic_assignments","euk_taxa.rds"))

####  Biodiversity calculations ----
sample_data <- read_csv(here::here("basic_data","sample_data.csv"))
tab2 <- readRDS(here::here("asv_tables","euk_combined_adjusted.rds"))
prep <- prepare_data(tab2, sample_data)
saveRDS(prep, here::here("processed_data","eukaryote_prepared.rds"))

#### Eukaryote taxonomy calculations ----
taxa <- readRDS(here::here("taxonomic_assignments","euk_taxa.rds"))
prep <- readRDS(here::here("processed_data","eukaryote_prepared.rds"))
prep$rare_tab[prep$rare_tab>1] <- 1
tar_tax_profile <- taxonomic_turnover_wrapper(prepared_data=prep, taxonomy = taxa, markername = "Eukaryotes", taxlevels = c("kingdom","supergroup","division","class","order","family","genus"),num_tax = 11)
saveRDS(tar_tax_profile,here::here("processed_data","eukaryote_taxon_profiles.rds"))

prep <- readRDS(here::here("processed_data","eukaryote_prepared.rds"))
taxa <- readRDS(here::here("taxonomic_assignments","euk_taxa.rds"))
tax_turnover <- readRDS(here::here("processed_data","eukaryote_taxon_profiles.rds"))

#### Eukaryote line graph ----
line_graphs <- overall_line_graphs(prep)
#line_graphs_compound <- plot_grid(plotlist=line_graphs, nrow=1)
saveRDS(line_graphs, here::here("partial_plots","eukaryote_line_graph_single.rds"))
#saveRDS(line_graphs_compound, here::here("partial_plots","tar_line_graph_compound.rds"))

#### NMDS plot ----
mdsplot <- plotmds2(prep)
saveRDS(mdsplot, here::here("partial_plots","eukaryote_nmds.rds"))

#### Eukaryote taxonomic plots ----
tax_turnover <- readRDS(here::here("processed_data","eukaryote_taxon_profiles.rds"))
...
saveRDS(plotz, here::here("partial_plots","eukaryote_raw_tax_plots.rds"))
tax_turn_plot <- plot_grid(plotlist=plotz, ncol=1, align = "v")
saveRDS(tax_turn_plot, here::here("partial_plots","eukaryote_combined_tax_plots.rds"))
...
bw_col <- readRDS(here::here("asv_tables_referencedata","bw_tar_seqtab.nochim_Both.rds"))
...
saveRDS(tab_m, here::here("asv_tables","euk_storage_and_biowide_otutable.rds"))

tab_m <- readRDS(here::here("asv_tables","euk_storage_and_biowide_otutable.rds"))
...
saveRDS(ratio_plots, here::here("partial_plots","eukaryote_ratio_plot.rds"))

#### KNN Site matching/classification ----
...
saveRDS(correct,here::here("processed_data","eukaryote_habitat_classification.rds"))
...
saveRDS(result, here::here("processed_data","eukaryote_habitat_classification.rds"))
